...
class MusicCog(commands.Cog):
    ...
    async def autocomplete_query(self, interaction: discord.Interaction, current: str):
        if not current:
            return []
        ytdlp_opts = {
            "format": "bestaudio/best",
        ...
        with yt_dlp.YoutubeDL(ytdlp_opts) as ytdl:
            try:
                info = ytdl.extract_info(f"ytsearch5:{current}", download=False)
    ...
    @app_commands.describe(query="Şarkı adı veya link")
    @app_commands.autocomplete(query=autocomplete_query)
